    ...
    <div class="container-fluid">
                ...
                <div class="col-xs-6 col-sm-2">
                    @foreach (config('lieplus.gender') as $key => $value)
                        <label class="control-label line-height-1 blue">
                            <input name="gender" type="radio" class="ace" value="{{ $key }}" @if (old('gender') ?? $resume->gender == $key) checked @endif required />
        ...
        <span class="input-group-addon">
          <i class="ace-icon fa fa-phone"></i>
                        ...
                        <input type="tel" id="mobile" name="mobile" value="{{ old('mobile') ?? $resume->mobile }}" required pattern="1(3|4|5|7|8)[0-9]{9}"/>
                    </div>
                    <span class="red">
        {{ $errors->first('mobile') }}
      </span>
                    ...
                    <div class="clearfix">
                        <select name="degree" id="degree">
                            @foreach (config('lieplus.degree') as $key => $value)
                                <option value="{{ $key }}"  @if (old('degree') ?? $resume->degree == $key) selected @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <span class="input-group-addon">
          <i class="ace-icon fa fa-calendar"></i>
                        ...
                        <input type="date" name="birthdate" id="birthdate" value="{{ old('birthdate') ?? $resume->birthdate }}" required max="{{ date('Y-m-d', time()) }}"/>
                        <span class="red">
        ...
        <span class="input-group-addon">
          <i class="ace-icon fa fa-calendar"></i>
                        ...
                        <input type="date" name="start_work_date" id="start_work_date" value="{{ old('start_work_date') ?? $resume->start_work_date }}" max="{{ date('Y-m-d', time()) }}" min="{{ date('Y-m-d', strtotime('-20 years')) }}" required />
                        <span class="red">
          {{ $errors->first('start_work_date') }}
            ...
            <div class="form-group">
                    ...
                    <div class="clearfix">
                        <select name="service_status" id="service_status">
                            @foreach (config('lieplus.service.status') as $key => $value)
                                <option value="{{ $key }}" @if($resume->stervice_status == $key) checked @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-xs-6 col-sm-2">
                    <div class="clearfix">
                        <input type="text" name="industry" id="industry" value="{{ old('industry') ?? $resume->industry }}" />
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-xs-6 col-sm-2">
                    <div class="clearfix">
                        <input type="text" name="position" id="position" value="{{ old('position') ?? $resume->position }}" />
                    </div>
                </div>
                    ...
                    <div class="clearfix">
                        <select name="salary" id="salary" class="col-xs-12 col-sm-4">
                            @foreach (config('lieplus.salary') as $key => $value)
                                <option value="{{ $key }}" @if (old('salary') ?? $resume->salary == $key) checked @endif>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
